...
export default function ClientChatScreen() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`chat-client-${id}-${Date.now()}`)
      .on(
        'postgres_changes',
        ...
        (payload) => {
          ...
          setMessages((prev) => {
            // Remove any pending stub with same content+sender, then add confirmed
  ...
  async function sendMessage() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Refetch to replace temp message with confirmed one (in case Realtime is not active)
  ...
  const renderItem = useCallback(({ item }: { item: Message }) => {
    ...
    return (
      <View style={[styles.bubble, isMe ? styles.bubbleMe : styles.bubbleThem, item._pending && { opacity: 0.6 }]}>
        {!isMe && <Text style={styles.bubbleSender}>Profissional</Text>}
